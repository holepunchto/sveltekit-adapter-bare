...
export default function (opts = {}) {
  ...
  return {
    ...
    async adapt(builder) {
      ...
      // Bare's packaged runtime aborts on dynamic import() from inside an app
      // bundle, so hoist every literal one (route nodes in the manifest, the
      // async_hooks stub) into a static import. Only laziness is lost.
      patch_dynamic_imports(join(out, 'server'))
...
/**
 * Rewrite `import("./x.js")` to `Promise.resolve(ns)` backed by a hoisted
 * `import * as ns from "./x.js"` in every bundled JS file. The lookbehind
 * keeps call sites like `obfuscated_import(...)` out of the match.
 * @param {string} dir
 */
function patch_dynamic_imports(dir) {
  const pattern = /(?<![\w$.])import\(\s*(["'])(\.\.?\/[^"']+)\1\s*\)/g
  ...
  for (const file of js_files(dir)) {
    ...
    const body = src.replace(pattern, (match, _quote, specifier, offset) => {
      // Leave JSDoc type imports like `import('./types.js').CspConfig` alone.
      const line = src.slice(src.lastIndexOf('\n', offset) + 1, offset)
      if (/^\s*(\*|\/\/)/.test(line)) return match

      let name = hoisted.get(specifier)
      if (!name) {
        name = `__hoisted_import_${hoisted.size}`
        hoisted.set(specifier, name)
      }
      return `Promise.resolve(${name})`
    })
    if (!hoisted.size) continue

    const imports = [...hoisted]
      .map(([specifier, name]) => `import * as ${name} from ${JSON.stringify(specifier)}`)
      .join('\n')
...
/**
 * Emit an ES module embedding every static file under client/ and
 * prerendered/ as base64. import.meta.asset() is a dead end on the packaged
 * Android runtime — the bundle maps assets to asset:/ URLs no module
 * protocol implements — and the files are small enough to ride along in the
 * bundle. The module exports two maps: { relPath → base64 }.
 * @param {string} out
 * @returns {string}
 */
function generate_assets_module(out) {
  ...
  const lines = (rels, sub) =>
    rels
      .map(
        (r) =>
          `\t${JSON.stringify(r)}: ${JSON.stringify(readFileSync(join(out, sub, r)).toString('base64'))}`
      )
      .join(',\n')
  ...
  return [
    '// AUTO-GENERATED by adapter-bare. Embeds every static asset, base64.',
    '',
    'export const client = {',
    lines(client, 'client'),
    '};',
